...
Sampling is without replacement and stratified by topic. The allocation
is as even as topic capacities allow; if one topic has too few
questions, its unused slots are redistributed across the other topics. A
local seed makes selection reproducible without changing the caller’s
random-number state. Filters limit the pool before sampling.

With `shuffle = FALSE` (the default), topics appear in the order
supplied to `topics`, and questions within each topic retain index
order. If `topics` is omitted, all selected questions retain index
order. Set `shuffle = TRUE` to randomise the final question order.
...
When `generated/` does not already contain a `_quarto.yml`, the build
creates one that makes it an isolated default Quarto project. This
prevents a surrounding website, book, or other parent Quarto project
from contributing its files or resources to the quiz output. An existing
`_quarto.yml` is preserved.

Student and teacher HTML/PDF documents include a table of contents by
default. To omit it from their generated `.qmd` wrappers and rendered
outputs, use:
